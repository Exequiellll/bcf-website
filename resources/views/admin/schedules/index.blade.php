    ...
    <div class="px-4 py-6 sm:px-0">
        <div class="mb-6">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-2">
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Schedules</h1>
                <a href="{{ route('admin.schedules.create') }}" class="inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 shadow-md hover:shadow-lg transition-all">
                    ...
                    <li>
                        <div class="px-4 py-4 sm:px-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                            <div class="flex-1 min-w-0">
                                <h3 class="text-base sm:text-lg font-medium text-gray-900 break-words">{{ $schedule->title }}</h3>
                                <p class="text-sm text-gray-500 mt-1 flex flex-wrap items-center gap-2">
                                    <span>{{ $schedule->day_of_week }} - {{ $schedule->start_time }}@if($schedule->end_time) to {{ $schedule->end_time }}@endif</span>
                            ...
                            <div class="flex space-x-3 sm:flex-shrink-0">
                                <a href="{{ route('admin.schedules.edit', $schedule->id) }}" class="text-blue-600 hover:text-blue-900 font-medium">Edit</a>
