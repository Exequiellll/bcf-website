...
<body class="bg-gradient-to-br from-gray-50 to-gray-100">
    ...
    <nav class="bg-white shadow-lg border-b border-gray-200" x-data="{ mobileOpen: false }">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                ...
                <div class="flex items-center space-x-4">
                    <a href="{{ route('home') }}" target="_blank" class="hidden sm:inline text-gray-600 hover:text-blue-600 text-sm font-medium transition-colors">
                        View Site
                    </a>
                    ...
                    <!-- Mobile menu button -->
                    <button @click="mobileOpen = !mobileOpen" class="sm:hidden inline-flex items-center justify-center p-2 rounded-md text-gray-600 hover:text-blue-600 hover:bg-gray-100 focus:outline-none">
        ...
        <!-- Mobile menu -->
        <div x-show="mobileOpen" x-cloak x-transition class="sm:hidden border-t border-gray-200 bg-white">
            <div class="px-2 pt-2 pb-3 space-y-1">
                <a href="{{ route('admin.dashboard') }}" class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('admin.dashboard') ? 'text-blue-600 bg-blue-50' : 'text-gray-600 hover:bg-gray-50' }}">Dashboard</a>
                <a href="{{ route('admin.announcements.index') }}" class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('admin.announcements.*') ? 'text-blue-600 bg-blue-50' : 'text-gray-600 hover:bg-gray-50' }}">Announcements</a>
                <a href="{{ route('admin.events.index') }}" class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('admin.events.*') ? 'text-blue-600 bg-blue-50' : 'text-gray-600 hover:bg-gray-50' }}">Events</a>
                <a href="{{ route('admin.schedules.index') }}" class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('admin.schedules.*') ? 'text-blue-600 bg-blue-50' : 'text-gray-600 hover:bg-gray-50' }}">Schedules</a>
                <a href="{{ route('admin.church-people.index') }}" class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('admin.church-people.*') ? 'text-blue-600 bg-blue-50' : 'text-gray-600 hover:bg-gray-50' }}">Team</a>
                <a href="{{ route('admin.livestream.index') }}" class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('admin.livestream.*') ? 'text-blue-600 bg-blue-50' : 'text-gray-600 hover:bg-gray-50' }}">Live Stream</a>
                <a href="{{ route('admin.inbox.index') }}" class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('admin.inbox.*') ? 'text-blue-600 bg-blue-50' : 'text-gray-600 hover:bg-gray-50' }}">
                    Inbox
                    @if($unreadCount > 0)
                        <span class="ml-2 bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ $unreadCount }}</span>
                    @endif
