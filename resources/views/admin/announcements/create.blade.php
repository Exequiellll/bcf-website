...
<div class="max-w-4xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
        ...
        <form method="POST" action="{{ route('admin.announcements.store') }}" class="bg-white shadow rounded-lg p-6">
            @csrf
            ...
            <div class="mb-4">
                ...
                <textarea name="content" id="content" rows="10" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('content') }}</textarea>
            </div>
            
            <div class="mb-4">
                <label for="event_date" class="block text-sm font-medium text-gray-700">Happens On (optional)</label>
                ...
                <p class="mt-1 text-xs text-gray-500">If this announcement is about something that happens on a specific date, set it here. After the date passes, it moves to "Past" for 7 days then auto-deletes.</p>
            </div>

            <div class="mb-4">
                <label for="published_at" class="block text-sm font-medium text-gray-700">Published Date</label>
                <input type="date" name="published_at" id="published_at" value="{{ old('published_at') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
            </div>
